...
Performs Partial Least Squares (PLS) regression using either the NIPALS
or SVD algorithm for component extraction. This is the main user-facing
function for computing PLS models. Internally, it delegates to either
[`NIPALS.pls()`](https://jesusbutforgaypeople.github.io/snazzieR/reference/NIPALS.pls.md)
or
[`SVD.pls()`](https://jesusbutforgaypeople.github.io/snazzieR/reference/SVD.pls.md).
...
- x:
  ...
  A numeric matrix or data frame of predictor variables (X), with
  dimensions n × p.

- y:

  A numeric matrix or data frame of response variables (Y), with
  dimensions n × q.

- n.components:

  Integer specifying the number of latent components (H) to extract. If
  NULL, defaults to the rank of `x`.
...
This function provides a unified interface for Partial Least Squares
regression. Based on the value of `calc.method`, it computes latent
variables using either:

- `"SVD"` — A direct method using the singular value decomposition of
  the cross-covariance matrix (\\X^\top Y\\).

- `"NIPALS"` — An iterative method that alternately estimates predictor
  and response scores until convergence.

The outputs from both methods include scores, weights, loadings,
